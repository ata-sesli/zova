...
const Trace = struct {
    ...
    fn callback(_: c_uint, context: ?*anyopaque, statement: ?*anyopaque, _: ?*anyopaque) callconv(.c) c_int {
        ...
        self.statements += 1;
        if (std.mem.indexOf(u8, sql, "select graph_key from") != null and std.mem.indexOf(u8, sql, "_zova_graphs") != null) self.graph_keys += 1;
        if (std.mem.indexOf(u8, sql, "_zova_graph_nodes") != null and std.mem.indexOf(u8, sql, "node_id") != null) self.node_resolutions += 1;
        if (std.mem.indexOf(u8, sql, "_zova_graph_edge_types") != null and std.mem.indexOf(u8, sql, "edge_type") != null) self.edge_type_resolutions += 1;
...
fn runGraph(arena: std.mem.Allocator, db: *zova.Database, store: []const u8) !void {
    ...
    for (ops) |op| {
        ...
        std.debug.print("scope=graph store={s} op={s} samples={d} p50_us={d:.3} p95_us={d:.3} traced_statements={d} graph_keys={d} node_resolutions={d} edge_type_resolutions={d} object_metadata={d} object_manifest={d}\n", .{ store, op, samples, percentile(timings, 50), percentile(timings, 95), trace.statements, trace.graph_keys, trace.node_resolutions, trace.edge_type_resolutions, trace.object_metadata, trace.object_manifest });
    }
}
...
fn runObject(arena: std.mem.Allocator, db: *zova.Database, store: []const u8) !void {
    ...
    const ops = [_][]const u8{ "metadata", "range", "reader" };
    for (ops) |op| {
        ...
        try objectOp(db, id, op);
        _ = c.sqlite3_trace_v2(db.sqlite_db.handle, 0, null, null);
        std.debug.print("scope=object store={s} op={s} samples={d} p50_us={d:.3} p95_us={d:.3} traced_statements={d} graph_keys={d} node_resolutions={d} edge_type_resolutions={d} object_metadata={d} object_manifest={d}\n", .{ store, op, samples, percentile(timings, 50), percentile(timings, 95), trace.statements, trace.graph_keys, trace.node_resolutions, trace.edge_type_resolutions, trace.object_metadata, trace.object_manifest });
    }
}
