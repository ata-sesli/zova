...
test "put mutates and reuses caller buffer between chunk inserts" {
    ...
    // no chunk binding from an earlier statement survives into later calls
    // or keeps referring to the reused buffer.
    const first_id = try db.putObject(&bytes);
    for (&bytes, 0..) |*byte, index| {
...
test "empty object put keeps zero-length chunk storage intact" {
    ...
    var path_buffer: [std.fs.max_path_bytes]u8 = undefined;
    const db_path = try testingDbPath(&path_buffer, tmp.sub_path[0..], "put-empty-object.zova");
...
test "trigger failure during chunk insert rolls back statement state cleanly" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();

    var path_buffer: [std.fs.max_path_bytes]u8 = undefined;
    const db_path = try testingDbPath(&path_buffer, tmp.sub_path[0..], "put-chunk-rollback.zova");
...
test "put inside caller transaction with bound store verifies stored bytes" {
    ...
    const main_path = try testingDbPath(&main_buffer, tmp.sub_path[0..], "put-chunk-txn-main.zova");
    const store_path = try testingDbPath(&store_buffer, tmp.sub_path[0..], "put-chunk-txn-objects.zova");
