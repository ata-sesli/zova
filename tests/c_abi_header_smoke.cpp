...
static_assert(ZOVA_VECTOR_ELEMENT_TYPE_F32 == 0, "vector element type values are stable");
static_assert(ZOVA_VECTOR_ELEMENT_TYPE_F16 == 1, "vector element type values are stable");
static_assert(ZOVA_VECTOR_ELEMENT_TYPE_I8 == 2, "vector element type values are stable");
...
int main() {
    ...
    zova_graph_edge_payload_results graph_edge_payloads = {};
    ...
    zova_vector_collection_options options = {
        ...
        ZOVA_VECTOR_ELEMENT_TYPE_F32,
    };
    int8_t i8_values[2] = {1, 2};
    zova_vector_values typed_values = {};
    typed_values.element_type = ZOVA_VECTOR_ELEMENT_TYPE_I8;
    typed_values.i8_values = i8_values;
    typed_values.values_len = 2;
    zova_vector_input vector_inputs[1] = {};
    vector_inputs[0].id = "chunk-001";
    vector_inputs[0].values = typed_values;
    ...
    create_collection_request.options = options;
    ...
    zova_graph_fresh_node_input fresh_node_input = {};
    fresh_node_input.node_id = "fresh";
    fresh_node_input.kind = "node";
    zova_graph_fresh_edge_input fresh_edge_input = {};
    fresh_edge_input.edge_type = "links";
    zova_graph_build_fresh_keyed_request fresh_build_request = {};
    fresh_build_request.graph_name = "fresh";
    fresh_build_request.nodes = &fresh_node_input;
    ...
    zova_graph_fresh_edge_payload_input fresh_payload_edge = {};
    fresh_payload_edge.edge_type = "links";
    zova_graph_build_fresh_prepared_keyed_with_payloads_request payload_build_request = {};
    payload_build_request.nodes = &fresh_node_input;
    payload_build_request.nodes_len = 1;
    payload_build_request.edges = &fresh_payload_edge;
    payload_build_request.edges_len = 1;
    const auto prepared_payload_build = &zova_graph_build_fresh_prepared_keyed_with_payloads;
    (void)prepared_payload_build;
    ...
    fresh_build_request.out_node_keys = &graph_node_key;
    fresh_build_request.out_node_keys_capacity = 1;
    fresh_build_request.out_edge_keys = &graph_edge_key;
    fresh_build_request.out_edge_keys_capacity = 1;
    ...
    zova_graph_edge_payload_get_many_request payload_get_request = {};
    payload_get_request.edge_keys = &graph_edge_key;
    payload_get_request.key_count = 1;
    payload_get_request.out_results = &graph_edge_payloads;
    zova_graph_edge_payload_replacement payload_replacement = {};
    payload_replacement.edge_key = graph_edge_key;
    zova_graph_edge_payload_replace_many_request payload_replace_request = {};
    payload_replace_request.replacements = &payload_replacement;
    payload_replace_request.replacement_count = 1;
    zova_fresh_build *fresh_builder = nullptr;
    zova_fresh_build_begin_request fresh_begin_request = {};
    fresh_begin_request.out_build = &fresh_builder;
    zova_fresh_value fresh_value = {};
    fresh_value.value_type = ZOVA_FRESH_VALUE_NULL;
    const char *fresh_columns[] = {"value"};
    zova_fresh_build_rows_request fresh_rows_request = {};
    fresh_rows_request.column_names = fresh_columns;
    fresh_rows_request.column_count = 1;
    ...
    const auto fresh_fts_rows = &zova_fresh_build_fts_rows;
    const auto fresh_graph = &zova_fresh_build_graph;
    const auto fresh_vectors = &zova_fresh_build_vectors;
    const auto fresh_finish = &zova_fresh_build_finish;
    const auto fresh_abort = &zova_fresh_build_abort;
    const auto fresh_destroy = &zova_fresh_build_destroy;
    (void)fresh_table_rows; (void)fresh_fts_rows; (void)fresh_graph; (void)fresh_vectors;
    (void)fresh_finish; (void)fresh_abort; (void)fresh_destroy;
    ...
    zova_graph_edge_payload_results_free(&graph_edge_payloads);
    ...
    return zova_database_close(db) == ZOVA_INVALID_ARGUMENT &&
                   ...
                   options.element_type == ZOVA_VECTOR_ELEMENT_TYPE_F32 &&
                   put_many_request.vectors_len == 1 &&
                   create_collection_request.options.element_type == ZOVA_VECTOR_ELEMENT_TYPE_F32 &&
                   put_request.values.values_len == 2 &&
                   get_request.out_vector == &vector &&
                   search_request.out_results == &search_results &&
                   info_request.out_info == &collection_info &&
                   list_request.out_list == &collection_list &&
